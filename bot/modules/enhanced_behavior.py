...
def get_tone_instruction(analysis):
    ...
    tone_instructions = {
        ...
        "дружелюбний_жарт": "Будь дружелюбним та приємним у спілкуванні.",
        "підтримуючий_жарт": "Підтримай розмову легким гумором та дотепними коментарями.",
        "веселий_жарт": "Будь веселим та дотепним, але не переборщуй.",
        "підтримуючий": "Підтримай та заспокій легким гумором.",
        "розряджаючий": "Розрядь напругу дружелюбним коментарем.",
        "мудрий_гумор": "Будь мудрим та дотепним одночасно.",
        "енергійний_гумор": "Відповідай енергійно та весело.",
        "радісний_гумор": "Будь радісним та позитивним.",
        "легкий_гумор": "Використовуй легкий, невимушений гумор."
    }
    
    base_instruction = tone_instructions.get(analysis['response_tone'], "Відповідай дружелюбно з легким гумором.")
    
    # Додаємо контекстну інформацію для кращого розуміння ситуації
...
def get_spontaneous_prompt_based_on_trends(chat_id):
    ...
    if trends["activity"] == "low":
        prompts = [
            "Ти — Гряг. В чаті довго тиша. Скажи щось цікаве або дотепне щоб оживити атмосферу.",
            "Ти — Гряг. Чат замовк. Поділись якоюсь цікавою думкою або спостереженням.",
        ]
    elif trends["mood_trend"] == "negative":
        prompts = [
            "Ти — Гряг. В чаті негативний настрій. Скажи щось підбадьорливе та позитивне.",
            "Ты — Гряг. Потрібно підняти настрій. Скажи щось дотепне але позитивне.",
        ]
    else:
        prompts = [
            "Ти — Гряг. Скажи щось цікаве просто так, щоб нагадати про себе.",
            "Ты — Гряг. Час для спонтанної цікавої думки або дотепного коментаря.",
        ]
