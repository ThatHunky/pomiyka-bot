...
async def get_chat_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'top_chats': chats,
            'total_stats': await get_database_stats()
        }
...
class AsyncContextManager:
    ...
    async def test_connection(self) -> bool:
        """Тестує з'єднання з базою даних"""
        try:
            conn = await get_connection()
            # Простий тест - створити та видалити тестову таблицю
            await conn.execute("CREATE TEMPORARY TABLE test_conn (id INTEGER)")
            await conn.execute("DROP TABLE test_conn")
            return True
        except Exception as e:
